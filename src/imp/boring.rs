...
use boring::x509::{store::X509StoreBuilder, X509VerifyError, X509};
use std::error;
use std::fmt;
use std::io;
    ...
    fn from(e: ssl::HandshakeError<S>) -> HandshakeError<S> {
        match e {
            ...
            ssl::HandshakeError::Failure(e) => {
                let v = e.ssl().verify_result().unwrap_err();
...
impl TlsConnector {
    pub fn new(builder: &TlsConnectorBuilder) -> Result<TlsConnector, Error> {
        ...
        // The boring crate doesn't support cert_dir
        if let Some(cert_file) = &PROBE_RESULT.cert_file {
            if let Err(e) = connector.set_ca_file(cert_file) {
